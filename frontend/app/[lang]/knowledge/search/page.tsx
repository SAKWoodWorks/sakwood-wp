...
export default function KnowledgeSearchPage({
  ...
  useEffect(() => {
    async function init() {
      ...
      if (queryParam) {
        await performSearch(queryParam, langParam, dict);
      }
    }
  ...
  const performSearch = async (searchQuery: string, locale: string, dict: any) => {
    setLoading(true);
    setSearched(true);

    const result = await searchKBArticles(searchQuery, locale);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (query.trim()) {
      router.push(`/${lang}/knowledge/search?q=${encodeURIComponent(query)}`);
      performSearch(query, lang, dictionary);
    }
  };
